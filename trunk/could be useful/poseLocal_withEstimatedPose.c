...
void estimatedPose(char xP, char yP, char thetaP) {

	  char cellX = xP;
	  char cellY = yP;
	  char cellTheta = thetaP;
		char averagingRadius = 6;
		float sumWeightedX = 0;
		float sumWeightedY = 0;
		float sumWeightedTheta = 0;
		float sumActivations = 0;
		for (char dX = -averagingRadius; dX < averagingRadius; dX++) {
			char x = getWrappedX(cellX + dX);
			for (char dY = -averagingRadius; dY < averagingRadius; dY++) {
				char y = getWrappedX(cellY + dY);
				for (char dTheta = -averagingRadius; dTheta < averagingRadius;
						dTheta++) {
					char theta = getWrappedTheta(cellTheta + dTheta);
					float activation = poseWorld.poseActivity[x].array2D[y][theta];
					sumWeightedX += activation * dX;
					sumWeightedY += activation * dY;
					sumWeightedTheta += activation * dTheta;
					sumActivations += activation;
				}
			}
		}

				estimatedX = (float) (cellX + (sumWeightedX / sumActivations))*0.8;
				estimatedY = (float) (cellY + (sumWeightedY / sumActivations))*0.8;
				estimatedTheta = (float) (cellTheta + (sumWeightedTheta / sumActivations))*0.8;
}
...
task main ()
{
  ...
  estimatedPose(poseWorld.maxActivatedCell.x, poseWorld.maxActivatedCell.y, poseWorld.maxActivatedCell.theta);
  /*displayMax();
